 ...
 */
public class BlockingAwarePrimitiveProxyClient extends DelegatingPrimitiveProxyClient {
  private final Executor executor;
  private final Map<Consumer<PrimitiveProxyClient.State>, Consumer<PrimitiveProxyClient.State>> stateChangeListeners = Maps.newConcurrentMap();
  private final Map<Consumer<PrimitiveEvent>, Consumer<PrimitiveEvent>> eventListeners = Maps.newConcurrentMap();

  public BlockingAwarePrimitiveProxyClient(PrimitiveProxyClient delegate, Executor executor) {
    super(delegate);
  ...
  public void addStateChangeListener(Consumer<PrimitiveProxyClient.State> listener) {
    Consumer<PrimitiveProxyClient.State> wrappedListener = state -> executor.execute(() -> listener.accept(state));
  ...
  public void removeStateChangeListener(Consumer<PrimitiveProxyClient.State> listener) {
    Consumer<PrimitiveProxyClient.State> wrappedListener = stateChangeListeners.remove(listener);
